function plotcoatPull_new(pos, loopsol, alpha, act, dash, neg)

coatPullSol = loopsol;

rad(:) = [coatPullSol(1,:,pos) coatPullSol(7,:,pos)];
height(:) = [coatPullSol(2,:,pos) coatPullSol(8,:,pos)];
mean(:) = [coatPullSol(4,:,pos) coatPullSol(10,:,pos)];
lam(:) = [coatPullSol(6,:,pos) coatPullSol(12,:,pos)];

diff = coatPullSol(8,end,pos) - coatPullSol(8,end,1); 


Sol(:,:) = coatPullSol(:,:,pos);

if neg == -1
    % Use this for normal plotcoatpullnew (fix z)
    %{
    Sol(2,:) = -Sol(2,:) +2 ;
    Sol(8,:) = -Sol(8,:)+2 ;
    %}
    ...
    Sol(2,:) = -Sol(2,:) +2 + diff ;
    Sol(8,:) = -Sol(8,:)+2 + diff;
    Sol(2,:) = Sol(2,:) - min(Sol(8,:));
    Sol(8,:) = Sol(8,:) - min(Sol(8,:));
end
...
if ~isempty(actArea)
    ...
    plot(Sol(7,:)*R0, Sol(8,:)*R0, ...
                  'Color', actColor, 'LineWidth', lineWidth);
    plot(-Sol(7,:)*R0, Sol(8,:)*R0, ...
                  ':','Color', actColor, 'LineWidth', lineWidth);
