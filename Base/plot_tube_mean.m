function plot_tube_mean(Sol, coatArea, x, R0, plotTitle, xLim, yLim, fArea)
...
hold on
scatter(Sol(7,:)*R0, Sol(8,:)*R0, pointsize, Sol(10,:)/R0);%plot(Sol(1,:)*R0, Sol(2,:)*R0,'Color', memColor, 'LineWidth', lineWidth);
hold on
colorbar
h = colorbar;
ylabel('Z (nm)', 'FontSize',fontsize, 'FontName', 'Helvetica');
set(get(h,'title'),'string','Mean Curvature (/nm)');
