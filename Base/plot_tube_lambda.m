function plot_tube_lambda(Sol, coatArea, x, R0, plotTitle, xLim, yLim, fArea)
...
pointsize = 10;
scatter(Sol(1,:)*R0, Sol(2,:)*R0, pointsize, Sol(6,:)*k0/R0^2);%plot(Sol(1,:)*R0, Sol(2,:)*R0,'Color', memColor, 'LineWidth', lineWidth);
hold on
scatter(Sol(7,:)*R0, Sol(8,:)*R0, pointsize, Sol(12,:)*k0/R0^2);%plot(Sol(1,:)*R0, Sol(2,:)*R0,'Color', memColor, 'LineWidth', lineWidth);
hold on
colorbar
h = colorbar;
set(get(h,'title'),'string','Surface Tension (pN/nm)');
